...
class ParserModel(nn.Module):
    def __init__(self, config):
        ...
        # 仅当用户在 config 中开启了预训练词向量时，才把权重拷进来并冻结。
        # 旧版直接 copy_(None) 会崩，这里加 None 判断以适配纯随机初始化的快速实验。
        ...
        if self.encoder_type == "lstm":
            ...
            raise ValueError(
                "未知 encoder_type: {!r}, 仅支持 'lstm' / 'transformer'".format(
                    self.encoder_type))
    ...
    def forward(self, words, ext_words, tags, masks):
        ...
        # TransformerEncoder 直接返回 batch_first 的 (B, L, output_dim),不需要 transpose
        if self.encoder_type == "lstm":
