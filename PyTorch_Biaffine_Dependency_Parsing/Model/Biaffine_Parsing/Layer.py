...
class LSTM(nn.LSTM):
    def reset_parameters(self):
        # 注意：新版 PyTorch 已把 nn.init.orthogonal / constant 标记为 deprecated，
        # 必须用带下划线的原地版本，否则会抛 RuntimeError 或 warning。
        for name, param in self.named_parameters():
            if "weight" in name:
                for i in range(4):
                    nn.init.orthogonal_(self.__getattr__(name)[self.hidden_size*i:self.hidden_size*(i+1),:])
            if "bias" in name:
