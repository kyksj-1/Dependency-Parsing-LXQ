...
This section develops the three pieces of mathematical machinery that underlie the empirical observations in Section~\ref{sec:experiment}. We cover the biaffine arc-and-relation scoring function, the rotary positional rotation used in SDPA-Large, and the Newton--Schulz orthogonalization step at the core of the Muon optimizer. All notation is consistent across subsections.
...
\paragraph{Loss.} The parser is trained with two cross-entropy terms, one for choosing a head and one for naming the dependency relation once the head is fixed. For token $i$, let $y^{\text{head}}_i \in \{0,\ldots,L\}$ be its gold head index, where $0$ denotes the artificial root, and let $y^{\text{rel}}_i \in \{1,\ldots,R\}$ be its gold relation label. The arc distribution is a row-wise softmax over all candidate heads,
\[
p^{\text{arc}}_{i\rightarrow j}
=
\frac{\exp S^{\text{arc}}_{i,j}}{\sum_{j'=0}^{L}\exp S^{\text{arc}}_{i,j'}} ,
\]
whereas the relation distribution is evaluated at the \emph{gold} head instead of the predicted head,
\[
p^{\text{rel}}_{i,y^{\text{head}}_i,r}
=
\frac{\exp S^{\text{rel}}_{i,y^{\text{head}}_i,r}}{\sum_{r'=1}^{R}\exp S^{\text{rel}}_{i,y^{\text{head}}_i,r'}} .
\]
This teacher-forced relation loss keeps the label classifier from learning through an incorrect arc decision during training. Over the set $\mathcal{T}$ of non-padding, non-punctuation tokens in a minibatch, the joint loss is
\begin{equation}
\label{eq:loss}
\mathcal{L} = \frac{1}{|\mathcal{T}|} \sum_{i\in\mathcal{T}} \Big[ -\log p^{\text{arc}}_{i \rightarrow y^{\text{head}}_i} - \log p^{\text{rel}}_{i, y^{\text{head}}_i, y^{\text{rel}}_i} \Big].
\end{equation}
...
SDPA-Large replaces sinusoidal positional encoding with the rotary positional encoding (RoPE) of \citet{su2021roformer}. RoPE treats each pair of consecutive coordinates inside a query or key vector as the real and imaginary parts of a complex number, then rotates the complex number by an angle proportional to token position. Concretely, let $\mathbf{q}_p \in \mathbb{R}^{d_h}$ denote the query vector at position $p$, with $d_h$ even. We split $\mathbf{q}_p$ into $d_h / 2$ pairs of coordinates indexed by $k \in \{0, 1, \ldots, d_h/2 - 1\}$, define the per-pair frequency
\begin{equation}
...
Equation~(\ref{eq:ns-iter}) bounds the spectral norm of $\bO_t$ near one even when the momentum buffer $\bM_{t+1}$ has a large norm. This explains why Muon partially rescues the unstable post-norm SDPA runs in Section~\ref{sec:experiment}. The parameter step is controlled mainly by $\eta$, which makes an overly large Muon learning rate visible immediately.
